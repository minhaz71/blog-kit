...
How to put Hemdox BlogKit on a server and update it safely from GitHub.
...
- **`version.json`** at the repo root is the single source of truth — the
  core version plus a version per tool (AI writers, link agent,
...
git init
git add .
git commit -m "Hemdox BlogKit 1.1.0"
git branch -M main
...
- **Build on the server:** leave `public/build` ignored and delete the
  workflow. `blogkit:update` runs `npm ci && npm run build` automatically when
...
# do NOT set BLOGKIT_ALLOW_UNSAFE_WIPE / BLOGKIT_ALLOW_DESTRUCTIVE
# (legacy SHOPKIT_ALLOW_* names still honoured as fallbacks)
BACKUP_CLOUD_REMOTE=gdrive:BlogKit-Backups   # see docs/BACKUPS.md
```

Run `php artisan blogkit:preflight` — it must pass with no critical issues.
...
2. On the server, either:
   - **Admin button:** Security → Updates → **Update Hemdox BlogKit** (Super Admin).
   ...
   - **SSH (equivalent, most reliable):** `bash deploy.sh` — or directly
     `php artisan blogkit:update`. Preview with `php artisan blogkit:update --dry-run`.
...
- **Additive migrations only.** `migrate:fresh`, `db:wipe`, `migrate:refresh/reset`,
  ...
  single command only with `BLOGKIT_ALLOW_DESTRUCTIVE=1` — legacy
  `SHOPKIT_ALLOW_DESTRUCTIVE=1` still works — then unset it).
...
## Security checklist (run `blogkit:preflight`)

APP_ENV=production · APP_DEBUG=false · HTTPS APP_URL · non-root DB user ·
APP_KEY set · backups writable + cloud remote set · `BLOGKIT_ALLOW_*` (and legacy `SHOPKIT_ALLOW_*`) unset ·
