<x-filament-panels::page>
    ...
    <div class="mlb">
        ...
        <div class="mlb-toolbar">
            ...
            <span class="mlb-count">{{ $images->total() }} of {{ $totalImages }} image(s)</span>
            <div class="mlb-views">
                <button type="button" class="{{ $view_mode === 'tiles' ? 'active' : '' }}" wire:click="setView('tiles')">
                ...
                <button type="button" class="{{ $view_mode === 'list' ? 'active' : '' }}" wire:click="setView('list')">
            ...
            <div class="mlb-edit">
                ...
                <div class="mlb-edit-grid">
                    <div class="mlb-preview">
                        ...
                        <div class="mlb-facts">
                            <span><strong>File:</strong> {{ $editing->filename }}</span>
                            @if($editing->dimensions)<span><strong>Dimensions:</strong> {{ $editing->dimensions }}</span>@endif
                            @if($editing->sizeKb !== null)<span><strong>Size:</strong> {{ number_format($editing->sizeKb) }} KB</span>@endif
                            <span>
                                <strong>Product:</strong>
                                @if($editing->model->product)
                                    <a class="mlb-link" href="{{ \App\Filament\Resources\ProductResource::getUrl('edit', ['record' => $editing->model->product]) }}">{{ $editing->model->product->name }}</a>
                                    @if($editing->isFeatured)<span class="mlb-flag mlb-flag-featured">featured</span>@endif
                                @else
                    ...
                    <div class="mlb-form">
                        @if($editing->lint !== [])
                            <div class="mlb-lint">⚠ {{ implode(' ', $editing->lint) }}</div>
                        @endif
                        <label class="mlb-field">
                            <span>Alt text (≤{{ \App\Services\Seo\ImageSeoRules::ALT_MAX }})</span>
                            <input type="text" wire:model="editAlt" maxlength="{{ \App\Services\Seo\ImageSeoRules::ALT_MAX }}" placeholder="Describe what the image shows…">
                        </label>
                        <label class="mlb-field">
                            <span>Title — hover tooltip (≤{{ \App\Services\Seo\ImageSeoRules::TITLE_MAX }})</span>
                            <input type="text" wire:model="editTitle" maxlength="{{ \App\Services\Seo\ImageSeoRules::TITLE_MAX }}">
                        </label>
                        <label class="mlb-field">
                            <span>Caption (≤{{ \App\Services\Seo\ImageSeoRules::CAPTION_MAX }})</span>
                            <input type="text" wire:model="editCaption" maxlength="{{ \App\Services\Seo\ImageSeoRules::CAPTION_MAX }}">
                        </label>
                        <label class="mlb-field">
                        ...
                        <label class="mlb-field">
                            ...
                            <input type="text" value="{{ $editing->url }}" readonly onclick="this.select()">
                        ...
                        <div class="mlb-actions-row">
                            <x-filament::button color="gray" wire:click="closeEdit">Cancel</x-filament::button>
                            <x-filament::button wire:click="save" wire:loading.attr="disabled">Save details</x-filament::button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        @if($view_mode === 'tiles')
            @if($images->isEmpty())
                <div class="mlb-panel"><div class="mlb-empty">No images{{ trim($search) !== '' ? ' matching “'.$search.'”' : '' }}.</div></div>
            @else
                <div class="mlb-grid">
                    @foreach($images as $image)
                        <button type="button" class="mlb-tile" wire:key="tile-{{ $image->id }}" wire:click="edit({{ $image->id }})">
                            <img src="{{ $image->url() }}" alt="" loading="lazy">
                            <div class="mlb-tile-meta">
                                <p class="mlb-tile-name">{{ $image->product?->name ?? basename($image->path) }}</p>
                                <p class="mlb-tile-sub">{{ basename($image->path) }}</p>
                                <div style="margin-top:.3rem; display:flex; gap:.3rem; flex-wrap:wrap">
                                    @if($image->product?->featured_image === $image->path)
                ...
                <table class="mlb-table">
                    ...
                    <tbody>
                        @forelse($images as $image)
                            <tr wire:key="row-{{ $image->id }}">
                                <td><img src="{{ $image->url() }}" alt="" class="mlb-thumb-sm" loading="lazy"></td>
                                <td>
                                    {{ basename($image->path) }}
                                    @if($image->product?->featured_image === $image->path)
                                        <span class="mlb-flag mlb-flag-featured">featured</span>
                                ...
                                <td>
                                    @if($image->product)
                                        <a class="mlb-link" href="{{ \App\Filament\Resources\ProductResource::getUrl('edit', ['record' => $image->product]) }}">{{ $image->product->name }}</a>
                                    @else
                                        <span class="mlb-muted">unassigned</span>
                                    @endif
                                </td>
                                <td>{{ $image->alt ?: '' }}@if(blank($image->alt))<span class="mlb-flag mlb-flag-noalt">missing</span>@endif</td>
                                <td class="mlb-muted">{{ $image->title ?: '—' }}</td>
                                <td><button type="button" class="mlb-link" style="background:none;border:0;cursor:pointer;font-size:.8rem" wire:click="edit({{ $image->id }})">Edit</button></td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="mlb-empty">No images{{ trim($search) !== '' ? ' matching “'.$search.'”' : '' }}.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
