<x-filament-panels::page>
    {{-- Self-contained styles: the admin panel ships Filament's precompiled
         CSS only, so this page brings its own (scoped, dark-mode aware),
         matching the AI usage & cost dashboard. --}}
    <style>
    ...
    <div class="aibm" wire:poll.3s>
        ...
        <div class="aibm-panel">
            <div class="aibm-status">
                <div class="aibm-status-left">
                    <span class="aibm-dot">
                        @if($running)
                            <span class="ping" style="background: {{ $statusColor }}"></span>
                        @endif
                        <span class="core" style="background: {{ $statusColor }}"></span>
                    </span>
                    <span class="aibm-state">{{ $batch->status }}</span>
                    <span class="aibm-provider">{{ \App\Models\AiImportBatch::PROVIDERS[$batch->provider] ?? $batch->provider }}</span>

                    @if($batch->status === 'processing')
                        <button type="button" class="aibm-btn amber" wire:click="pauseBatch" wire:loading.attr="disabled">⏸ Pause</button>
                    @endif
                    @if(in_array($batch->status, ['paused', 'stopped']))
                        <button type="button" class="aibm-btn green" wire:click="resumeBatch" wire:loading.attr="disabled">▶ Resume</button>
                    @endif
                    @if(in_array($batch->status, ['processing', 'paused']))
        ...
        <div class="aibm-cards">
            <div class="aibm-card">
                ...
                <p class="aibm-card-value" style="color: #0d9488">{{ $batch->progressPercent() }}%</p>
                <p class="aibm-card-sub">{{ $batch->done_items }} of {{ $batch->total_items }} products</p>
            </div>
            <div class="aibm-card">
                <div class="aibm-card-glow" style="background: linear-gradient(135deg, #10b981, #14b8a6)"></div>
                <p class="aibm-card-label">Published</p>
                <p class="aibm-card-value" style="color: #059669">{{ $batch->done_items }}</p>
            ...
            <div class="aibm-card">
                ...
                <p class="aibm-card-value" style="color: {{ $batch->failed_items > 0 ? '#dc2626' : '#9ca3af' }}">{{ $batch->failed_items }}</p>
                <p class="aibm-card-sub">{{ $batch->failed_items > 0 ? 'open the batch to retry' : 'none' }}</p>
            </div>
            <div class="aibm-card">
                <div class="aibm-card-glow" style="background: linear-gradient(135deg, #f59e0b, #ef4444)"></div>
                <p class="aibm-card-label">Spent</p>
                <p class="aibm-card-value" style="color: #d97706">${{ number_format($spend, 4) }}</p>
                <p class="aibm-card-sub">USD, this batch</p>
            </div>
            <div class="aibm-card">
                <div class="aibm-card-glow" style="background: linear-gradient(135deg, #10b981, #14b8a6)"></div>
                <p class="aibm-card-label">Cache savings</p>
                <p class="aibm-card-value" style="color: #059669">${{ number_format($cacheSaved, 4) }}</p>
                <p class="aibm-card-sub">{{ $cacheHitRate }}% of input read from prompt cache</p>
            </div>
        </div>
        ...
        {{-- ── Alerts ───────────────────────────────────────────────── --}}
        @if($batch->error)
            <div class="aibm-alert err"><span><strong>Batch error:</strong> {{ $batch->error }}</span></div>
        @endif

        @if($batch->total_items === 0 && in_array($batch->status, ['pending', 'processing']))
        ...
        @if($workerStalled && $pendingItems > 0)
            <div class="aibm-alert warn">
                <span>
                    <strong>Processing looks stalled</strong> — {{ $pendingItems }} product(s) are still waiting.
        ...
        <div class="aibm-grid">
            {{-- Item pipeline --}}
            <div class="aibm-panel">
                <div class="aibm-panel-head"><h2>{{ $this->record->kind === 'blog' ? 'Articles' : 'Products' }} ({{ $items->count() }})</h2></div>
                <ul class="aibm-items">
                    @forelse($items as $item)
                        <li class="aibm-item">
                            <div style="min-width: 0">
                                <p class="aibm-item-name">{{ $item->row['name'] ?? "Item #{$item->id}" }}</p>
                                @if($item->error)
                                    <p class="aibm-item-err" title="{{ $item->error }}">{{ $item->error }}</p>
                                @elseif($item->status === 'needs_review' && $item->review_summary)
                                    <p class="aibm-item-err" title="{{ $item->review_summary }}">{{ $item->open_issues }} open issue(s): {{ $item->review_summary }}</p>
                                @elseif($item->passes_done > 0 && in_array($item->status, ['reviewing', 'published', 'linked']))
                                    <p class="aibm-item-meta">QA passes: {{ $item->passes_done }}@if($item->preview_url) · <a href="{{ $item->preview_url }}" target="_blank" style="color:#0f766e">view content</a>@endif</p>
                                @endif
                            </div>
                            @php
                            ...
                            @endphp
                            <span class="aibm-pill {{ $pillClass }} {{ $pulse }}">{{ $item->status === 'linked' ? 'published + linked' : str_replace('_', ' ', $item->status) }}</span>
                        </li>
                    @empty
            ...
            <div class="aibm-panel">
                ...
                <div class="aibm-feed">
                    @forelse($feed as $log)
                        <div class="aibm-log {{ $log->level }}">
                            <span class="aibm-log-time">{{ $log->created_at->format('H:i:s') }}</span>
                            <span class="aibm-log-stage">{{ $log->stage }}</span>
                            <span class="aibm-log-msg">{{ $log->message }}</span>
                        </div>
                    @empty
