    ...
    <div class="abc">
        @if($enabled)
            <div class="abc-note on">
                ...
                @if($settingsUrl) <a href="{{ $settingsUrl }}">Edit the flow →</a> @endif
            </div>
        @else
            <div class="abc-note off">
                ✕ Abandoned cart recovery is OFF. No reminders are being sent.
                @if($settingsUrl) <a href="{{ $settingsUrl }}">Turn it on →</a> @endif
            </div>
        @endif
        ...
        <div class="abc-cards">
            <div class="abc-card">
                ...
                <p class="abc-card-value warn">{{ $stats['open'] }}</p>
                <p class="abc-card-sub">{{ $stats['value_at_risk'] }} at risk</p>
            </div>
            <div class="abc-card">
                <p class="abc-card-label">Reminders sent</p>
                <p class="abc-card-value">{{ $stats['reminders_sent'] }}</p>
                <p class="abc-card-sub">to {{ $stats['emailed'] }} shopper(s)</p>
            </div>
            <div class="abc-card">
                <p class="abc-card-label">Recovered orders</p>
                <p class="abc-card-value good">{{ $stats['recovered'] }}</p>
                <p class="abc-card-sub">{{ $stats['recovery_rate'] }}% recovery rate</p>
            </div>
            <div class="abc-card">
                <p class="abc-card-label">Recovered revenue</p>
                <p class="abc-card-value good">{{ $stats['recovered_revenue'] }}</p>
                <p class="abc-card-sub">from recovered carts</p>
            </div>
        </div>

        <div class="abc-panel">
            <div class="abc-panel-head"><h2>Reminder sequence</h2><span>{{ count($stages) }} stage(s)</span></div>
            <div class="abc-flow">
                @forelse($stages as $i => $stage)
                    <div class="abc-step">
                        <span class="abc-chip">{{ $stage['label'] }}</span>
                        @if($i < count($stages) - 1)<span class="abc-arrow">→</span>@endif
                    </div>
                @empty
                    <span style="color:#9ca3af; font-size:.85rem;">No stages configured.</span>
                @endforelse
                @if(count($stages))<div class="abc-step"><span class="abc-arrow">→</span><span style="color:#9ca3af; font-size:.8rem; font-weight:600;">exit</span></div>@endif
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach($rows as $row)
                            <tr>
                                <td>
                                    {{ $row['email'] ?? '—' }}
                                    <span class="abc-badge {{ $row['guest'] ? 'guest' : 'member' }}">{{ $row['guest'] ? 'Guest' : 'Member' }}</span>
                                    @if($row['name'] !== '—')<div style="color:#9ca3af; font-size:.75rem;">{{ $row['name'] }}</div>@endif
                                </td>
                                <td>{{ $row['items'] }}</td>
                                <td>{{ $row['value'] }}</td>
                                <td class="abc-stage">{{ $row['stage'] }} / {{ $row['stage_count'] }}</td>
                                <td>{{ $row['last_active'] }}</td>
                                <td>{{ $row['last_reminder'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
