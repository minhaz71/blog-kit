    ...
    <div class="aiud" wire:poll.10s>
        ...
        <div class="aiud-cards">
            @foreach ([
                ...
                <div class="aiud-card">
                    <div class="aiud-card-glow" style="background: linear-gradient(135deg, {{ $card['from'] }}, {{ $card['to'] }})"></div>
                    <div class="aiud-card-head">
                        <span class="aiud-chip" style="background: linear-gradient(135deg, {{ $card['from'] }}, {{ $card['to'] }})">
                            <svg fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $card['icon'] }}" />
                            </svg>
                        </span>
                        <p class="aiud-label">{{ $card['label'] }}</p>
                    </div>
                    <p class="aiud-amount" style="color: {{ $card['from'] }}">${{ number_format((float) $card['data']->cost, 4) }}</p>
                    <p class="aiud-sub">USD total cost</p>
                    <p class="aiud-meta">
                        {{ number_format((int) $card['data']->requests) }} requests ·
                        {{ number_format((int) $card['data']->input) }} in / {{ number_format((int) $card['data']->output) }} out tokens
                    </p>
                </div>
            @endforeach
            ...
            <div class="aiud-card aiud-savings">
                ...
                <p class="aiud-amount">${{ number_format($cacheSavings, 4) }}</p>
                <p class="aiud-sub">USD saved</p>
                <p class="aiud-meta">{{ number_format((int) $allTime->cached) }} cached tokens billed at the cache rate</p>
            </div>
        </div>
        ...
        <div class="aiud-panel">
            ...
            <div class="aiud-scroll">
                <table class="aiud-table">
                    ...
                    <tbody>
                        @forelse ($byProduct as $index => $row)
                            <tr>
                                <td class="aiud-dim">{{ $index + 1 }}</td>
                                <td class="aiud-strong">{{ $row->name }}</td>
                                <td>
                                    @if($row->batch)
                                        <span class="aiud-badge">{{ $row->batch }}</span>
                                    @else
                                        <span class="aiud-dim">—</span>
                                    @endif
                                </td>
                                <td class="aiud-right">{{ number_format($row->requests) }}</td>
                                <td class="aiud-right">{{ number_format($row->input) }}</td>
                                <td class="aiud-right aiud-green">{{ number_format($row->cached) }}</td>
                                <td class="aiud-right">{{ number_format($row->output) }}</td>
                                <td class="aiud-right aiud-cost">${{ number_format($row->cost, 4) }}</td>
                                <td class="aiud-right">
                                    @if($row->product_id)
                                        <a href="/admin/products/{{ $row->product_id }}/edit" class="aiud-edit">
                ...
                <table class="aiud-table">
                    ...
                    <tbody>
                        @forelse ($byModel as $row)
                            @php $share = (float) $allTime->cost > 0 ? (float) $row->cost / (float) $allTime->cost * 100 : 0; @endphp
                            <tr>
                                <td style="text-transform: capitalize">{{ $row->provider }}</td>
                                <td class="aiud-strong">
                                    {{ $row->model }}
                                    @unless(\App\Models\AiUsageLog::isPriced($row->model))
                                ...
                                <td>
                                    <div class="aiud-share">
                                        <div class="aiud-share-track"><div class="aiud-share-fill" style="width: {{ round($share) }}%"></div></div>
                                        <span>{{ number_format($share, 1) }}%</span>
                                    </div>
                                </td>
                                <td class="aiud-right">{{ number_format((int) $row->requests) }}</td>
                                <td class="aiud-right">{{ number_format((int) $row->input) }}</td>
                                <td class="aiud-right aiud-green">{{ number_format((int) $row->cached) }}</td>
                                <td class="aiud-right">{{ number_format((int) $row->output) }}</td>
                                <td class="aiud-right aiud-cost">${{ number_format((float) $row->cost, 4) }}</td>
                            </tr>
                        @empty
                    ...
                    <tbody>
                        @forelse ($recent as $log)
                            <tr>
                                ...
                                <td>{{ $log->batch?->name ?? '—' }}</td>
                                <td><span class="aiud-badge {{ $log->purpose === 'write' ? 'write' : ($log->purpose === 'review' ? 'review' : '') }}">{{ $log->purpose }}</span></td>
                                <td>{{ $log->model }}</td>
                                <td class="aiud-right">
                                    {{ number_format($log->input_tokens) }} /
                                    <span class="aiud-green">{{ number_format($log->cached_tokens) }}</span> /
                                    {{ number_format($log->output_tokens) }}
                                </td>
                                <td class="aiud-right aiud-strong">${{ number_format((float) $log->cost, 5) }}</td>
                            </tr>
                        @empty
