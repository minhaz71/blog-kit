    ...
    <div class="sc" wire:poll.30s>
        ...
        <div class="sc-top">
            <div class="sc-panel sc-score">
                <div class="sc-ring" style="background: conic-gradient({{ $ringColor }} {{ $score * 3.6 }}deg, rgba(0,0,0,.08) 0deg);">
                    <div class="sc-ring-inner">
                        <div class="sc-ring-num" style="color: {{ $ringColor }}">{{ $score }}</div>
                        <div class="sc-ring-grade">Grade {{ $audit['grade'] }}</div>
                    </div>
                </div>
                <div class="sc-score-sub">{{ $audit['passed'] }}/{{ $audit['total'] }} hardening checks passed</div>
            </div>
            ...
            <div class="sc-cards">
                @foreach ([
                    ...
                    <div class="sc-card">
                        <div class="sc-card-num" style="color: {{ $card['c'] }}">{{ $card['n'] }}</div>
                        <div class="sc-card-label">{{ $card['l'] }}</div>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="sc-panel">
                ...
                @forelse ($failed as $c)
                    <div class="sc-check">
                        ...
                        <div class="sc-check-body">
                            <div class="sc-check-label">{{ $c['label'] }}<span class="sc-sev {{ $c['severity'] }}">{{ $c['severity'] }}</span></div>
                            <div class="sc-check-fix">{{ $c['fix'] }}</div>
                        </div>
                    </div>
                @empty
                ...
                @foreach ($passed as $c)
                    <div class="sc-check">
                        ...
                        <div class="sc-check-body"><div class="sc-check-label" style="color:#6b7280">{{ $c['label'] }}</div></div>
                    </div>
                @endforeach
            </div>
            ...
            <div style="display:flex; flex-direction:column; gap:1.5rem;">
                ...
                <div class="sc-panel">
                    ...
                    @forelse ($topIps as $row)
                        <div class="sc-row"><span class="sc-mono">{{ $row->ip_address }}</span><span class="sc-badge">{{ number_format($row->hits) }} hits</span></div>
                    @empty
                ...
                <div class="sc-panel">
                    ...
                    @forelse ($topRules as $row)
                        <div class="sc-row"><span>{{ str_replace('_', ' ', $row->rule) }}</span><span class="sc-badge">{{ number_format($row->hits) }}</span></div>
                    @empty
        ...
        <div class="sc-grid2">
            ...
            <div class="sc-panel">
                ...
                @forelse ($events as $e)
                    <div class="sc-event">
                        <span class="sc-dot {{ $e->severity }}"></span>
                        <div style="min-width:0; flex:1;">
                            <div style="font-weight:500;">{{ $e->title }}</div>
                            @if($e->description)<div style="color:#6b7280; font-size:.78rem; margin-top:.1rem;">{{ \Illuminate\Support\Str::limit($e->description, 120) }}</div>@endif
                        </div>
                        <span class="sc-time">{{ $e->created_at?->diffForHumans() }}</span>
                    </div>
                @empty
            ...
            <div style="display:flex; flex-direction:column; gap:1.5rem;">
                ...
                <div class="sc-panel">
                    ...
                    <div class="sc-row"><span>Threat IPs loaded</span><span class="sc-badge">{{ number_format($stats['threat_ips']) }}</span></div>
                    <div class="sc-row"><span>Last updated</span><span class="sc-time">{{ $threatUpdatedAt ? \Illuminate\Support\Carbon::parse($threatUpdatedAt)->diffForHumans() : 'never — run “Update threat blocklist”' }}</span></div>
                </div>

                {{-- Dependency vulnerabilities --}}
                <div class="sc-panel">
                    <div class="sc-panel-head">Dependency vulnerabilities</div>
                    @if($dependencies && $dependencies['ran'])
                        @if($dependencies['advisories'] === 0)
                            <div class="sc-row" style="color:#059669;"><span>✅ No known CVEs in dependencies</span><span class="sc-time">{{ \Illuminate\Support\Carbon::parse($dependencies['checked_at'])->diffForHumans() }}</span></div>
                        @else
                            @foreach ($dependencies['packages'] as $p)
                                <div class="sc-row"><span class="sc-mono">{{ $p['package'] }}</span><span class="sc-sev high">{{ $p['severity'] }}</span></div>
                            @endforeach
                        @endif
                    @else
