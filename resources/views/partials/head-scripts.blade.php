{{-- Analytics / tag manager — injected as high in <head> as practical
     (Admin → SEO settings → Google tag & tracking). IDs are format-validated
     on save (GTM-…, G-…/AW-…/UA-…), so they're safe in the JS context. --}}
@php
    $gtmId = trim((string) setting('seo.google_tag_manager_id'));
    $gtagId = trim((string) setting('seo.google_tag_id'));
    $customHead = trim((string) setting('seo.custom_head_code'));
@endphp
@if($gtmId !== '')
    {{-- Google Tag Manager --}}
    <script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','{{ $gtmId }}');</script>
    {{-- End Google Tag Manager --}}
@endif
@if($gtagId !== '')
    {{-- Google tag (gtag.js) --}}
    <script async src="https://www.googletagmanager.com/gtag/js?id={{ $gtagId }}"></script>
    <script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','{{ $gtagId }}');</script>
@endif
@if($customHead !== '')
    {!! $customHead !!}
@endif
