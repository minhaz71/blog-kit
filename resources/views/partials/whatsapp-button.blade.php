...
@if($waEnabled && $waNumber !== '')
    ...
    @endphp
    {{-- Floating WhatsApp button. Static per settings, so it is safe on
         cached guest pages; reveal + position are driven by wa.js. --}}
    <a href="{{ $waHref }}"
       class="wa-fab wa-{{ $waPosition }}"
       target="_blank"
       rel="noopener nofollow"
       data-wa-delay="{{ $waDelay }}"
       aria-label="{{ $waGreeting !== '' ? $waGreeting : 'Chat on WhatsApp' }}">
        ...
        @if($waGreeting !== '')
            <span class="wa-fab-label">{{ $waGreeting }}</span>
        @endif
    </a>
@endif
