...
- **NEVER break the owner's one-click / network Update** — site owners are non-technical and have no VPS/SSH access; `blogkit:update` (and the hub's remote Update action) MUST keep working with zero manual steps:
  - **Do NOT rewrite published git history** (no force-push that changes existing commit hashes). A rewrite orphans every deployed checkout and forces a manual `git reset --hard` on each server. If commit-message/author cleanup is ever required, do it going FORWARD only (new commits), never retroactively. Prefer plain, no-co-author commit messages from the start.
  - Keep `blogkit:update` self-healing: mandatory full backup first, then update, then automatic rollback (code + DB) on any failure. It already auto-handles git `safe.directory`, runs Composer through the current PHP binary (never a stray 7.4), falls back to `reset --hard origin/<branch>` when a fast-forward is impossible, and treats `npm` as optional (committed `public/build` is the fallback). Preserve all of these when touching the updater.
  - Any change to the updater must be BACKWARD-COMPATIBLE: the site is still running the PREVIOUS updater when the new one is fetched, so the old code path must be able to pull the new code (don't rely on the new updater to install itself).
